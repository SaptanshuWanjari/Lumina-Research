...
def _fallback_role_model(role: str) -> str:
    if role == "planner":
...
def _build_llm(owner_user_id: str, role: str) -> GeminiService | OllamaService:
...
def analyzer_llm(owner_user_id: str) -> GeminiService | OllamaService:
    return _build_llm(owner_user_id, "analyzer")


def writer_llm(owner_user_id: str) -> GeminiService | OllamaService:
